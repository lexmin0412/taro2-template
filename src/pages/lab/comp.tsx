...
class Comp extends Component {
	constructor(props) {
		super(props)
		Meta.setTitle('底层基础组件，用于其他页面和组件继承')
		this.state = {
			type: 'image',
			imageList: [
		...
		return (
			<View className='comp-page'>
				...
				{type === 'card' ||
					(type === 'paging' && (
						<View className='demo-page-item'>
							卡片组件
							{imageList.map(item => {
								return <HdCard key={item}>{item}</HdCard>
							})}
						</View>
					))}
				...
				{type === 'modal' && (
					<View className='demo-page-item'>
						弹窗组件
						<Button onClick={this.showModal.bind(this, 'center')}>
							点击展示弹窗（居中弹窗）
						</Button>
						<Button onClick={this.showModal.bind(this, 'bottom')}>
							点击展示弹窗（底部弹窗）
						</Button>
						<HdModal
